...
function updateUI() {
    ...
    const countryDisplay = document.getElementById('current-country-display');
    if (countryDisplay) countryDisplay.textContent = currentCountry;
...
function updateStartupUI() {
    ...
    let startupListHTML = ''; // Buffer
    playerStartups.forEach((startup, index) => {
        let lvl = startup.level || 1;
        // Cerca la generazione dell'HTML dentro updateStartupUI e sostituisci i bottoni così:
startupListHTML += `
    <li class="mb-2 p-2.5 bg-white rounded-lg border border-gray-200 shadow-sm flex flex-col gap-1.5">
        ...
        <div class="flex justify-between items-center pt-1">
            ...
            <div class="flex gap-1">
                <button onclick="sellStartup(${index})" class="btn bg-red-500 hover:bg-red-600 text-white text-[10px] uppercase font-bold py-1 px-2 !rounded">
                    Vendi
                </button>
                <button onclick="openStartupQuest(${index})" class="btn bg-blue-500 hover:bg-blue-600 text-white text-[10px] uppercase font-bold py-1 px-2 !rounded">
                    🚀 Quest
                </button>
            </div>
        </div>
    ...
    startupListEl.innerHTML = startupListHTML;
}
...
function updateBankUI() {
    ...
    loansListEl.innerHTML = loansListHTML;
}