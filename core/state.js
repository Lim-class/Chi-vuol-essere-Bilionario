...
let globalInflation = 1.0; // Moltiplicatore base dell'inflazione
...
const taxSystems = {
    "Italia": {
        name: "Italia",
        fee: 5000,
        unemployedFixedTax: 50, // <-- NUOVO: Estratto per essere influenzato dall'inflazione
        exchangeTaxRate: 0.02, 
        calculateTax: (salaryAnnuale) => {
            // Sostituito il vecchio "50" con il riferimento dinamico
            if (salaryAnnuale === 0) return taxSystems["Italia"].unemployedFixedTax * 12; 
    ...
    "Stati Uniti": {
        name: "Stati Uniti",
        fee: 15000,
        unemployedFixedTax: 150, // <-- NUOVO
        exchangeTaxRate: 0.04, 
        calculateTax: (salaryAnnuale) => {
            if (salaryAnnuale === 0) return taxSystems["Stati Uniti"].unemployedFixedTax * 12;
            let federalTax = 0;
    ...
    "Svizzera": {
        name: "Svizzera",
        fee: 25000,
        unemployedFixedTax: 350, // <-- NUOVO
        exchangeTaxRate: 0.03, 
        calculateTax: (salaryAnnuale) => {
            if (salaryAnnuale === 0) return taxSystems["Svizzera"].unemployedFixedTax * 12;
    ...
    "Emirati Arabi (Dubai)": {
        ...
        fee: 50000,
        unemployedFixedTax: 0, 
        exchangeTaxRate: 0.06, 
        calculateTax: (salaryAnnuale) => {
            return 0; 
        }
    }
