...
function sellStartup() {
    if (playerStartups.length > 0) {
        ...
        const sharesValue = (lastStartup.sharesOwned || 0) * lastStartup.shareValue;
        const enterpriseValue = lastStartup.money * (1 + Math.random() * 0.5);
        ...
        showMessage(
            "Azienda Venduta", 
            `Hai venduto con successo <strong>${lastStartup.productName}</strong>!<br><br>` +
            `• Ricavo Liquidazione Quote: <span class="text-green-600 font-semibold">${formatCurrency(sharesValue)}</span><br>` +
            `• Valore Cassa/Patrimonio aziendale: <span class="text-green-600 font-semibold">${formatCurrency(enterpriseValue)}</span><br>` +
            `<hr class="my-2 border-gray-200">` +
            `• <strong>Totale Accreditato:</strong> <span class="text-indigo-600 font-bold">${formatCurrency(totalEarnings)}</span>`
        );
    } else {
        showMessage("Errore", "Non possiedi alcuna startup o azienda da vendere.");
    }
}
