function takeLoan() {
    ...
    // NUOVO: Doppio controllo di sicurezza contro i valori negativi inseriti da HTML
    if (isNaN(amount) || amount <= 0) return showMessage("Errore", "Inserisci un importo valido.");
    if (isNaN(duration) || duration <= 0) return showMessage("Errore", "Inserisci una durata valida in mesi (es. 12, 60).");
