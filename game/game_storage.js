...
function serializeGameState() {
    return {
        ...
        player: {
            ...
            currentCountry: currentCountry, // <-- NUOVO
            lastRelocationDate: lastRelocationDate.getTime() // <-- NUOVO (Salvato in timestamp sicuro)
        },
        economy: {
...
function deserializeGameState(data) {
    try {
        ...
        currentCountry = data.player.currentCountry || "Italia";
        lastRelocationDate = data.player.lastRelocationDate ? new Date(data.player.lastRelocationDate) : new Date(2020, 0, 1);
